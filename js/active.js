(function ($) {
    ...
    $(document).on('click', '.imovel', function () {
        document.location.href = 'imovel.php?id=' + $(this).attr('id') + '&finalidade=' + $(this).attr('finalidade');
    }).on('click', '.page-link', function () {
        $('#page').val($(this).html());
        $('#advanceSearch').submit();
    }).on('click', '#btnLogarInterno', function () {
        var formData = new FormData($("#formLoginInterno")[0]);
        $.ajax({
            type: 'POST',
            url: 'api/loginInterno.php',
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
            success: function (data) {
                if (data == 'ok') {
                    $('.searchbtn').find('i').css('color', 'greenyellow');
                    $('.searchbtn').find('i')[0].class = 'fa fa-sign-out';
